...
import { BTC, deploy_service, events_logs } from "./helpers.js";

describe("BitcoinOracle", () => {
  it("constructor sets owner, minimum_fee and deployed_block", async () => {
    const { oracle, admin, min_fee } = await deploy_service();
  ...
  it("request_update reverts when the fee is below the minimum", async () => {
    const { oracle, alice, min_fee } = await deploy_service();
    await assert.rejects(
      oracle.write.request_update([BTC], { account: alice.account, value: min_fee - 1n }),
      /fee too low/
    );
  });

  it("request_update succeeds and emits update_requested", async () => {
    const { oracle, alice, min_fee, public_client } = await deploy_service();
    const hash = await oracle.write.request_update([BTC], { account: alice.account, value: min_fee });
    const logs = await events_logs(public_client, oracle.abi, hash, "update_requested");
    assert.equal(logs.length, 1);
    assert.equal(logs[0].args.BTC_addr.toLowerCase(), BTC.toLowerCase());
    assert.equal(logs[0].args.fee, min_fee);
  });

  it("push_balance is restricted to the owner", async () => {
    const { oracle, alice } = await deploy_service();
    await assert.rejects(
  ...
  it("push_balance stores the balance and emits balance_updated", async () => {
    const { oracle, public_client } = await deploy_service();
    const hash = await oracle.write.push_balance([BTC, 12_345n]);
    const logs = await events_logs(public_client, oracle.abi, hash, "balance_updated");
    assert.equal(logs.length, 1);
  ...
  it("get_balance returns 0 for an unknown address", async () => {
    const { oracle } = await deploy_service();
  ...
  it("withdraw_fees moves accumulated fees to the recipient", async () => {
    const { oracle, alice, bob, min_fee, public_client } = await deploy_service();
